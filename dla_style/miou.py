...
class RunningConfusionMatrix():
    ...
    def __init__(self, labels, ignore_label=255, reduce=False):
    ...
    def update_matrix(self, ground_truth, output):
        ...
        # prepare for tensor use
        _, prediction = output.max(1)
        prediction = prediction.view(1, -1)
        ground_truth = ground_truth.view(1, -1)
        prediction = prediction[ground_truth!=self.ignore_label]
        ground_truth = ground_truth[ground_truth!=self.ignore_label]
        if self.reduce:
            prediction[prediction>1] = 1
            ground_truth[ground_truth>1] = 1

        prediction = prediction.cpu().detach().data.numpy().flatten()
        ground_truth = ground_truth.cpu().detach().data.numpy().flatten()
    ...
    def compute_current_mean_intersection_over_union(self):
        ...
        intersection_over_union = intersection / union.astype(np.float32)
        sorted_iou = np.sort(intersection_over_union)
        top_1 = sorted_iou[-1]
        # top_5 = np.mean(sorted_iou[-5:]) # TODO
        top_5 = 0

        mean_intersection_over_union = np.mean(intersection_over_union)
        
        return mean_intersection_over_union * 100, top_1 * 100, top_5 * 100